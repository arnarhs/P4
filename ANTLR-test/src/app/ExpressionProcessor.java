...
public class ExpressionProcessor {
		...
		for (Statement e : _list) {
			if (e instanceof VariableDeclaration) {
				...
				while (repeats > 0) {
					...
					//HashMap<String, Color> colorScheme = generateColorScheme(ssaResults.get(0).getSpecies().keySet());
	...
	private HashMap<String, Color> generateColorScheme(Set<String> keySet) {
		    ...
		    HashMap<String, Color> speciesColor = new HashMap<String, Color>();
		    
		    int i = 1; 
		    for(String elem : keySet) {
		    	speciesColor.put(elem, 
		    			Color.decode(colors.get(i)));
		    			/*new Color(
		    			Integer.valueOf( colors.get(i).substring( 1, 3 ), 16 ),
		                Integer.valueOf( colors.get(i).substring( 3, 5 ), 16 ),
		                Integer.valueOf( colors.get(i).substring( 5, 7 ), 16 ) ));*/
		    }
